...
#' @docType class
#' @section Objects from the Class: NpdeRes objects are created during a call to
#'   \code{\link{npde}} or \code{\link{autonpde}} as the "results" slot in a
#'   NpdeObject object. An NpdeRes object contains the following slots:
#'
#'   \describe{ \item{res}{a dataframe containing the results. Columns include
#'   id (group), xobs (observed X), yobs (observed Y), cens (indicator for
#'   censored data), as well as the actual results: ypred (model population
#'   predictions), pd (prediction discrepancies), npde (normalised prediction
#'   distribution errors), ycomp (completed data), ydobs (decorrelated observed
#'   data).} \item{N}{number of subjects} \item{ntot.obs}{total number of
#'   non-missing observations} \item{ploq}{a vector giving the probability that
#'   a given observation is LOQ, according to the model} \item{icens}{index of
#'   (non-missing) censored observations} \item{not.miss}{a vector of boolean
#'   indicating for each observation whether it is missing (FALSE) or available
#'   (TRUE)} \item{pd.sim}{pd computed for a number of simulated datasets
#'   (optional, used to obtain prediction intervals on the distribution of pd)}
#'   \item{npde.sim}{npde computed for a number of simulated datasets (optional,
#'   used to obtain prediction intervals on the distribution of npde)} }
#' @section Methods: \describe{ \item{print(npde.res):}{Prints a summary of
#'   object npde.res} \item{show(npde.res):}{Prints a short summary of object
#'   npde.res} \item{showAll(npde.res):}{Prints a detailed summary of object
#'   npde.res} \item{plot(npde.res):}{Plots the data in npde.res. More details
#'   can be found in \code{\link{plot.NpdeRes}}}
#'   \item{summary(npde.res):}{Returns a summary of object npde.res in list
#'   format} }
...
#'
#' Plots distribution and scatterplots for the npde in a NpdeRes object. 
#' Users are advised to use the plot() function on the NpdeObject object 
#' resulting from a call to npde() or autonpde() instead of trying to plot 
#' only the results element of this object.
#' 
...
#' \item{a histogram of the npde}{the shape of the normal distribution 
#' is also shown}
...
#'  distribution; for these last two graphs, we plot the lines corresponding 
#'  to y=0 	and to the 5\% and 95\% critical value of the normal distribution 
#'  delimiting a 90\% prediction interval for the npde}
#' }
...
#' @method gofTest numeric
#' @export

gofTest.numeric<-function(object,which="npde",parametric=TRUE, ...) {
...
# test de variance pour 1 echantillon
# chi=s2*(n-1)/sigma0 et test de H0={s=sigma0} vs chi2 a n-1 df
#    if(parametric) {
...
#    }
	...
	res$p.value<-myres
	res$nobs<-n1
#	if(verbose) print.gofTest(res, which=which)
	invisible(res)
}

#' @method gofTest NpdeRes
#' @export
...
gofTest.NpdeRes<-function(object,which="npde",parametric=TRUE, ...) {
# Performs test on the selected variable (one of npde, pd or npd)
	...
	res<-gofTest(npde,which=which,parametric=parametric)
	if(verbose)
		print.gofTest(res, which=which)
	invisible(res)
}

#' Prints a summary of a gofTest result

print.gofTest<-function(object, which="npde", ...) {
	cat("---------------------------------------------\n")
