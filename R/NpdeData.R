...
#'   dataset} \item{name.group}{character string giving the name of the grouping
#'   term (ID)} \item{name.predictor}{character string giving the name of the
#'   predictor (X)} \item{name.response}{character string giving the name of the
#'   response (Y)} \item{name.cens}{character string giving the name of the
#'   censoring indicator} \item{name.mdv}{character string giving the name of
#'   the missing data indicator} \item{name.covariates}{vector of character
#'   string giving the name(s) of the covariates} \item{name.ipred}{character
#'   string giving the name of the individual predictions}
#'   \item{units}{(optional) a list with the units for X, Y, and covariates}
#'   \item{data}{a dataframe containing the data} \item{N}{number of subjects}
...
#'   \item{not.miss}{a vector of boolean indicating for each observation whether
#'   it is missing (FALSE) or available (TRUE)} \item{loq}{the censoring value}
#'   }
...
#'   \item{setPlotOptions(npde.data):}{Sets options for graphs of npde.data
...
#' @docType class
#' @section Objects from the Class: NpdeSimData objects are created by
#'   associating an NpdeData object with matching simulated data, and they
#'   contain the following slots.
#'
#'   \describe{ \item{name.simdata}{character string giving the name of the
#'   dataset} \item{nrep}{number of replications)} \item{datsim}{a dataframe
...
#'   call to \code{\link{npde}} or \code{\link{autonpde}}, an additional column
#'   ydsim (decorrelated replicated data) will be added.} }
#' @section Methods: \describe{ \item{print(npde.simdata):}{Prints a summary of
#'   object npde.simdata} \item{show(npde.simdata):}{Prints a short summary of
#'   object npde.simdata} \item{showAll(npde.simdata):}{Prints a detailed
#'   summary of object npde.simdata} }
#' @seealso \code{\link{npde}}, \code{\link{autonpde}}
#' @keywords classes
#' @examples
#'
...
#'
#' @name showAll
#' @aliases showAll showAll.NpdeData showAll,NpdeData-method showAll.NpdeSimData
#'   showAll,NpdeSimData-method  showAll.NpdeRes  showAll.NpdeObject
...
#' @method setPlotOptions NpdeData

setPlotOptions.NpdeData<-function(object, ...) {
...
#'
#' This function is used to replace graph options (available in the prefs slot
#' of the NpdeData object) for plotting NpdeData objects
#'
...
#' @details See documentation for a list of available options. During
#'   replacement, invalid (including misspelled) options will raise warnings.
...
#' @details The default plot is a spaghetti plot of all the data, with a line
#'   joining the observations for each subject. If censored data is present, it
#'   is shown with a different symbol and colour.
...
##################################################################################
# @usage readNpdeData(object, header, sep, na.strings, detect)
...
#' @param detect a boolean; if TRUE, automatic recognition of names will be
#'   attempted to detect necessary items (longitudinal data structure, missing
#'   data and censoring information)
#' @return an object of class \code{"\linkS4class{NpdeData}"}
#' @keywords internal

setMethod("readNpdeData","NpdeData",
...
#' Read simulated data
#' 
#' Creates an NpdeSimData object containing the simulated data, either from disk
#' or from a dataframe
#'
#' @param header boolean indicating whether the file has a header (mandatory if
#'   detect is TRUE)
...
setMethod("readNpdeSimData","NpdeSimData",
...
#'
#' This function is used to create a NpdeData object, representing a
#' longitudinal data structure, and fill it with data from a dataframe or a file
#' on disk
#'
#' @usage
...
#' @param name.data name of the file containing the observed data, or a
#'   dataframe containing the observed data
#' @param sep field separator (for files on disk)
#' @param na.strings strings to be considered as indicating NA
#' @param header boolean indicating whether the file has a header (mandatory if
#'   detect is TRUE)
#' @param name.group name/number of the column in the observed data containing
#'   the patient ID (if missing and detect is TRUE, columns named id, subject or
#'   sujet (regardless of case) will be assumed to contain this information)
#' @param name.predictor name/number of the column in the observed data
#'   containing the independent variable X (if missing and detect is TRUE,
#'   columns named xobs, time, dose, x, temps, tim (regardless of case) will be
#'   assumed to contain this information)
#' @param name.response name/number of the column in the observed data
#'   containing the dependent variable Y (if missing and detect is TRUE, columns
#'   named yobs, response, resp, conc, concentration (regardless of case) will
#'   be assumed to contain this information)
#' @param name.miss name/number of the column containing information about
#'   missing data (MDV) (if missing and detect is TRUE, column called mdv or
#'   miss (regardless of case) will be assumed to contain this information)
#' @param name.cens name/number of the column containing information about
#'   censored data (cens) (if missing and detect is TRUE, column with a name
#'   containing cens (regardless of case) will be assumed to contain this
#'   information)
...
#'   (ipred)  (if missing and detect is TRUE, column with a name containing
#'   ipred (regardless of case) will be assumed to contain this information)
#' @param units a list with components x, y and cov (optional), specifying the
#'   units respectively for the predictor (x), the response (y), and the
#'   covariates (a vector of length equal to the number of covariates). Units
#'   will default to (-) if not given.
#' @param detect a boolean controlling whether automatic recognition of columns
#'   in the dataset is on, defaults to TRUE
#' @param verbose whether to print warning messages, defaults to FALSE (set to
#'   TRUE to check how data is being handled)
#'
...
#  showAll(x)
  ...
  x1<-readNpdeData(x,header=header,sep=sep,na.strings=na.strings,detect=detect, verbose=verbose)
...
npdeSimData<-function(npde.data,name.simdata,verbose=FALSE) {
	...
	x<-readNpdeSimData(x1,verbose=verbose)
